...
if (analog) {
  ...
  function setActive(directionName) {
    ...
    if (directionName && targets[directionName]) {
      ...
      if (chip) {
        chip.innerHTML = `
          <strong>${targets[directionName].label}</strong>
          <small>Release to open</small>
        `;
      }
    } else {
      resetChip();
  ...
  analog.addEventListener("pointermove", e => {
    ...
    analog.style.transform = `
      translate3d(${dx}px, ${dy}px, 68px)
      rotateX(${-dy / 7}deg)
      rotateY(${dx / 7}deg)
      scale(1.025)
    `;
