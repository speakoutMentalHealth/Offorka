...
if (portrait) {
  ...
  function setActive(direction) {
    ...
    if (direction) {
      ...
      if (label) label.textContent = `Release to open ${labels[direction]}`;
    } else if (label) {
      label.textContent = "Tap to call · Drag to explore";
    }
  }
  ...
  portrait.addEventListener("pointermove", (event) => {
    ...
    portrait.style.transform =
      `translate(${dx}px, ${dy}px) rotateX(${-dy / 7}deg) rotateY(${dx / 7}deg)`;
